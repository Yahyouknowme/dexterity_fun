...
import {AnchorProvider as $hgUW1$AnchorProvider, Program as $hgUW1$Program} from "@project-serum/anchor";
import {getAssociatedTokenAddress as $hgUW1$getAssociatedTokenAddress, TOKEN_PROGRAM_ID as $hgUW1$TOKEN_PROGRAM_ID, createAssociatedTokenAccountInstruction as $hgUW1$createAssociatedTokenAccountInstruction} from "@solana/spl-token";
import {PublicKey as $hgUW1$PublicKey, Connection as $hgUW1$Connection, Transaction as $hgUW1$Transaction, Keypair as $hgUW1$Keypair, SystemProgram as $hgUW1$SystemProgram, ComputeBudgetProgram as $hgUW1$ComputeBudgetProgram} from "@solana/web3.js";
import {EventQueueHeader as $hgUW1$EventQueueHeader, Slab as $hgUW1$Slab, EventQueue as $hgUW1$EventQueue, MarketState as $hgUW1$MarketState} from "@bonfida/aaob";
...
(0, $hgUW1$bnjs).prototype.toJSON = function() {
    return (0, $hgUW1$bnjs).prototype.toString.call(this, 10);
};

const $149c1bd638913645$var$WebSocket = $hgUW1$isomorphicws;

$parcel$global.Buffer = $parcel$global.Buffer || $hgUW1$Buffer;
Object.defineProperty((0, $hgUW1$EventQueueHeader), "LEN", {
    configurable: true,
    writable: true,
    value: 33
});
const $149c1bd638913645$var$DEX_ID = new (0, $hgUW1$PublicKey)("FUfpR31LmcP1VSbz5zDaM7nxnH55iBHkpwusgrnhaFjL");
const $149c1bd638913645$var$INSTRUMENTS_ID = new (0, $hgUW1$PublicKey)("8981bZYszfz1FrFVx7gcUm61RfawMoAHnURuERRJKdkq");
const $149c1bd638913645$var$RISK_ID = new (0, $hgUW1$PublicKey)("92wdgEqyiDKrcbFHoBTg8HxMj932xweRCKaciGSW3uMr");
// @ts-ignore
const $149c1bd638913645$var$DEX_IDL = (0, (/*@__PURE__*/$parcel$interopDefault($de7ee92a39635655$exports)));
// @ts-ignore
const $149c1bd638913645$var$INSTRUMENTS_IDL = (0, (/*@__PURE__*/$parcel$interopDefault($b7e293d8159cb031$exports)));
// @ts-ignore
const $149c1bd638913645$var$RISK_IDL = (0, (/*@__PURE__*/$parcel$interopDefault($a9cec026d4677f08$exports)));
const $149c1bd638913645$var$SENTINEL = 0;
const $149c1bd638913645$var$UNINITIALIZED = "11111111111111111111111111111111";
const $149c1bd638913645$var$SYSTEM_PROGRAM_ID = "11111111111111111111111111111111";
const $149c1bd638913645$var$MPG_SIZE = 143912;
const $149c1bd638913645$var$TRG_SIZE = 64272;
const $149c1bd638913645$var$MAX_OUTRIGHTS = 128;
const $149c1bd638913645$var$MAX_COMPUTE_UNITS = 1400000; // 1.4m is solana's max
const $149c1bd638913645$var$MAX_CANCELS_PER_TX = 8;
let $149c1bd638913645$var$rpc2manifest = new Map(); // maps rpc url to manifest (AAOB, DEX, FEES, RISK, MPGs, products, orderbooks, etc.)
let $149c1bd638913645$var$account2WebSocket = new Map(); // maps rpc:account to websocket
const $149c1bd638913645$var$bnSqrt = (num)=>{
    if (num.lt(new (0, $hgUW1$bnjs)(0))) throw new Error("Sqrt only works on non-negtiave inputs");
    if (num.lt(new (0, $hgUW1$bnjs)(2))) return num;
    const smallCand = $149c1bd638913645$var$bnSqrt(num.shrn(2)).shln(1);
    const largeCand = smallCand.add(new (0, $hgUW1$bnjs)(1));
    if (largeCand.mul(largeCand).gt(num)) return smallCand;
    else return largeCand;
};
class $149c1bd638913645$var$ReliableWebSocket {
    ref = 1;
    eventQueueSeqNum = 0;
    constructor(socket){
        this.socket = socket;
...
class $149c1bd638913645$var$Manifest {
    constructor(fields){
        this.fields = fields;
    ...
    setWallet(wallet) {
        ...
        const connection = new (0, $hgUW1$Connection)(this.fields.rpc, confirmOptions.preflightCommitment);
        const provider = new (0, $hgUW1$AnchorProvider)(connection, wallet, confirmOptions);
        let dexProgram = $149c1bd638913645$var$createDexProgram(provider);
        let instrumentsProgram = $149c1bd638913645$var$createInstrumentsProgram(provider);
        let riskProgram = $149c1bd638913645$var$createRiskProgram(provider);
    ...
    static GetRiskAndFeeSigner(mpg) {
        return (0, $hgUW1$PublicKey).findProgramAddressSync([
            mpg.toBuffer()
        ], new (0, $hgUW1$PublicKey)($149c1bd638913645$var$DEX_ID))[0];
    }
    getRiskS(mpg) {
        return (0, $hgUW1$PublicKey).findProgramAddressSync([
            $149c1bd638913645$require$Buffer.from("s", "utf-8"),
            mpg.toBuffer()
        ], new (0, $hgUW1$PublicKey)(this.fields.risk_id))[0];
    }
    getRiskR(mpg) {
        return (0, $hgUW1$PublicKey).findProgramAddressSync([
            $149c1bd638913645$require$Buffer.from("r", "utf-8"),
            mpg.toBuffer()
        ], new (0, $hgUW1$PublicKey)(this.fields.risk_id))[0];
    }
    static async GetATAFromMPGObject(mpg, wallet) {
        return await (0, $hgUW1$getAssociatedTokenAddress)(mpg.vaultMint, wallet);
    }
    accountSubscribe(pk, parseDataFn, onUpdateFn, useCache = true) {
        const pkStr = pk.toBase58();
        const key = this.fields.rpc + ":" + pkStr;
        let socket = new $149c1bd638913645$var$WebSocket($149c1bd638913645$var$toWebSocket(this.fields.rpc));
        ...
        socket.addEventListener("message", (async (event)=>{
            ...
            return;
            onUpdateFn(await parseDataFn($149c1bd638913645$require$Buffer.from(msg.params.result.value.data[0], "base64"), this), msg.params.result.context.slot);
        }).bind(this));
        let rws = new $149c1bd638913645$var$ReliableWebSocket(socket);
    ...
    static GetMarkPrice(markPrices, productKey) {
        for (const mp of markPrices.array){
            if (mp.productKey.equals(productKey)) return $149c1bd638913645$var$Manifest.FromFastInt(mp.markPrice.value);
        }
        return $149c1bd638913645$var$Fractional.Nan();
    }
    static GetMarkPriceOracleMinusBookEwma(markPrices, productKey) {
        for (const mp of markPrices.array){
            if (mp.productKey.equals(productKey)) return $149c1bd638913645$var$Manifest.FromFastInt(mp.oracleMinusBookEwma.value);
        }
        return $149c1bd638913645$var$Fractional.Nan();
    }
    static FromFastInt(bn) {
        return new $149c1bd638913645$var$Fractional(bn, new (0, $hgUW1$bnjs)(6));
    }
    async getMPGFromData(data) {
        // @ts-ignore
        return await $149c1bd638913645$var$Manifest.GetMPGFromData(this.fields.dexProgram, data);
    }
    ...
    static GetProductsOfMPG(mpg) {
        ...
        for (const p of mpg.marketProducts.array){
            i++;
            if ($149c1bd638913645$var$productStatus(p, mpg.marketProducts.array) === "uninitialized") continue;
            m.set($149c1bd638913645$var$bytesToString($149c1bd638913645$var$productToMeta(p).name), {
                index: i,
                product: p
            });
        }
    ...
    static GetActiveProductsOfMPG(mpg) {
        ...
        for (const p of mpg.marketProducts.array){
            i++;
            if ($149c1bd638913645$var$productStatus(p, mpg.marketProducts.array) !== "initialized") continue;
            m.set($149c1bd638913645$var$bytesToString($149c1bd638913645$var$productToMeta(p).name), {
                index: i,
                product: p
            });
    ...
    async getDerivativeMetadataFromData(data) {
        // @ts-ignore
        return await $149c1bd638913645$var$Manifest.GetDerivativeMetadataFromData(this.fields.instrumentsProgram, data);
    }
    ...
    getMarkPricesAccount(mpg) {
        return (0, $hgUW1$PublicKey).findProgramAddressSync([
            $149c1bd638913645$require$Buffer.from("mark_prices", "utf-8"),
            mpg.toBuffer()
        ], new (0, $hgUW1$PublicKey)(this.fields.risk_id))[0];
    }
    ugh(str) {
        return new (0, $hgUW1$PublicKey)(str);
    }
    ...
    async getBook(product, marketState) {
        const offset = $149c1bd638913645$var$Fractional.From(product.metadata.priceOffset);
        const tickSize = $149c1bd638913645$var$Fractional.From(product.metadata.tickSize);
        ...
        for (const order of bidsSlab.items(true))bids.push({
            quantity: new $149c1bd638913645$var$Fractional(order.baseQuantity, baseDecimals),
            price: $149c1bd638913645$var$Manifest.aaobOrderToDexPrice(order, tickSize, offset),
        ...
        for (const order1 of asksSlab.items(true))asks.push({
            quantity: new $149c1bd638913645$var$Fractional(order1.baseQuantity, baseDecimals),
            price: $149c1bd638913645$var$Manifest.aaobOrderToDexPrice(order1, tickSize, offset),
    ...
    static aaobOrderToDexPrice(aaobOrder, tickSize, offset) {
        return new $149c1bd638913645$var$Fractional(aaobOrder.getPrice().shrn(32), new (0, $hgUW1$bnjs)(0)).mul(tickSize).sub(offset);
    }
    static orderIdToDexPrice(id, tickSize, offset) {
        return new $149c1bd638913645$var$Fractional(id.shrn(96), new (0, $hgUW1$bnjs)(0)).div(tickSize).sub(offset);
    }
    ...
    streamBooks(product, marketState, onBookFn, onMarkPricesFn = null) {
        ...
        const bidsSocket = this.accountSubscribe(marketState.bids, async (data, manifest)=>{
            bids = [];
            const slab = (0, $hgUW1$Slab).deserialize(data, marketState.callBackInfoLen);
        ...
        if (onMarkPricesFn !== null) {
            ...
            for (const [pk, { pubkey: pubkey , mpg: mpg  }] of this.fields.mpgs){
                for (let [productName, { index: index , product: product1  }] of $149c1bd638913645$var$Manifest.GetProductsOfMPG(mpg)){
                    const meta = $149c1bd638913645$var$productToMeta(product1);
    ...
    streamTrades(product, marketState, onTradesFn) {
        const offsetFrac = $149c1bd638913645$var$Fractional.From(product.metadata.priceOffset);
        const socket = this.accountSubscribe(marketState.eventQueue, async (data, manifest)=>{
            const eventQueue = (0, $hgUW1$EventQueue).parse(marketState.callBackInfoLen, data);
            ...
            for (const fill of fills){
                const baseQty = $149c1bd638913645$var$Fractional.New(fill.baseSize, product.metadata.baseDecimals);
                // TODO idk why typescript doesn't believe "EventFill | EventOut" has the field "quoteSize"
                // @ts-ignore
                const quoteQty = $149c1bd638913645$var$Fractional.New(fill.quoteSize, product.metadata.baseDecimals);
                trades.push({
                    price: quoteQty.div(baseQty).mul($149c1bd638913645$var$Fractional.From(product.metadata.tickSize)).sub(offsetFrac),
                    quantity: baseQty,
    ...
    async getTRGsOfOwner(owner, marketProductGroup = null) {
        ...
        const filters = [
            {
                dataSize: $149c1bd638913645$var$TRG_SIZE
            },
            {
                memcmp: {
                    offset: 48,
    ...
    async closeTrg(marketProductGroup, traderRiskGroup) {
        ...
        {
            const tx = new (0, $hgUW1$Transaction)().add(await dexProgram.instruction.closeTraderRiskGroup({
                accounts: {
    ...
    async createTrg(marketProductGroup) {
        ...
        const riskStateAccount = new (0, $hgUW1$Keypair)();
        const traderRiskGroup = new (0, $hgUW1$Keypair)();
        const [traderFeeAccount, traderFeeAccountBump] = (0, $hgUW1$PublicKey).findProgramAddressSync([
            marketProductGroup.toBuffer(),
            traderRiskGroup.publicKey.toBuffer(),
            mpg.feeModelConfigurationAcct.toBuffer()
        ], this.fields.fees_id);
        const riskAndFeeSigner = $149c1bd638913645$var$Manifest.GetRiskAndFeeSigner(marketProductGroup);
        {
            // create trg account ix + intialize trg ix
            const rentExemptionAmount = await connection.getMinimumBalanceForRentExemption($149c1bd638913645$var$TRG_SIZE);
            const tx = new (0, $hgUW1$Transaction)().add(await dexProgram.account.traderRiskGroup.createInstruction(traderRiskGroup, $149c1bd638913645$var$TRG_SIZE)).add(await dexProgram.instruction.initializeTraderRiskGroup({
                accounts: {
                    owner: wallet.publicKey,
                    ...
                    systemProgram: (0, $hgUW1$SystemProgram).programId
                }
            }));
            try {
                let { blockhash: blockhash  } = await connection.getRecentBlockhash();
    ...
    async fetchOrderbooks(marketProductGroup = null) {
        ...
        for (const [k, { pubkey: pubkey , mpg: mpg , orderbooks: orderbooks , covarianceMetadata: covarianceMetadata  }] of this.fields.mpgs){
            ...
            for (let [productName, { index: index , product: product  }] of $149c1bd638913645$var$Manifest.GetActiveProductsOfMPG(mpg)){
                // await new Promise(_ => { setTimeout(100); });
                const meta = $149c1bd638913645$var$productToMeta(product);
                let marketState = null;
                try {
                    marketState = await (0, $hgUW1$MarketState).retrieve(this.fields.connection, meta.orderbook, confirmOptions.preflightCommitment);
                } catch (e) {
        ...
        for (const [k, { pubkey: pubkey , mpg: mpg , orderbooks: orderbooks , covarianceMetadata: covarianceMetadata  }] of this.fields.mpgs){
            for (let [productName, { index: index , product: product  }] of $149c1bd638913645$var$Manifest.GetActiveProductsOfMPG(mpg)){
                ...
                if (meta.orderbook.equals(orderbook)) {
                    const marketState = await (0, $hgUW1$MarketState).retrieve(this.fields.connection, orderbook, confirmOptions.preflightCommitment);
                    result = marketState;
    ...
    async updateOrderbooks(marketProductGroup) {
        ...
        for (let [productName, { index: index , product: product  }] of $149c1bd638913645$var$Manifest.GetActiveProductsOfMPG(mpg)){
            ...
            try {
                const marketState = await (0, $hgUW1$MarketState).retrieve(this.fields.dexProgram.provider.connection, meta.orderbook, "processed");
                orderbooks.set(meta.orderbook.toBase58(), marketState);
            } catch  {
    ...
    static GetRiskNumber(data, offset, size, isSigned = true) {
        if (isSigned) return new (0, $hgUW1$bnjs)(data.slice(offset, offset + size), undefined, "le").fromTwos(size * 8);
        return new (0, $hgUW1$bnjs)(data.slice(offset, offset + size), undefined, "le");
    }
    getStds(marketProductGroup) {
        ...
        const numActiveProducts = $149c1bd638913645$var$Manifest.GetRiskNumber(covarianceMetadata, offset, 8, false);
        offset += 8;
        let stds = new Map();
        for(let i = 0; i < numActiveProducts; i++){
            const pubkey = new (0, $hgUW1$PublicKey)(covarianceMetadata.slice(offset + 32 * i, offset + 32 * (i + 1)));
            const std = $149c1bd638913645$var$Manifest.FromFastInt($149c1bd638913645$var$Manifest.GetRiskNumber(covarianceMetadata, offset + $149c1bd638913645$var$MAX_OUTRIGHTS * 32 + 16 * i, 16, true));
            stds.set(pubkey.toBase58(), std);
        }
        return stds;
    }
}
async function $149c1bd638913645$var$getManifest(rpc, useCache = false, wallet) {
    const key = wallet ? wallet.publicKey + ":" + rpc : ":" + rpc;
    console.debug("getting manifest", key);
    if (useCache && $149c1bd638913645$var$rpc2manifest.has(key)) {
        console.debug("using cache to get manifest", key);
        return $149c1bd638913645$var$rpc2manifest.get(key);
    }
    const confirmOptions = {
        preflightCommitment: "processed"
    };
    const connection = new (0, $hgUW1$Connection)(rpc, confirmOptions.preflightCommitment);
    const provider = new (0, $hgUW1$AnchorProvider)(connection, wallet, confirmOptions);
    let dexProgram = $149c1bd638913645$var$createDexProgram(provider);
    let instrumentsProgram = $149c1bd638913645$var$createInstrumentsProgram(provider);
    let riskProgram = $149c1bd638913645$var$createRiskProgram(provider);
    ...
    let fees_id = null;
    let risk_id = null;
    const accounts = await connection.getParsedProgramAccounts($149c1bd638913645$var$DEX_ID, {
        filters: [
            {
                dataSize: $149c1bd638913645$var$MPG_SIZE
            }
        ]
    });
    const mpgs = new Map();
    for (const [i, { account: account , pubkey: pubkey  }] of accounts.entries()){
        const mpg = await $149c1bd638913645$var$Manifest.GetMPGFromData(dexProgram, account.data);
        risk_id = mpg.riskEngineProgramId;
        fees_id = mpg.feeModelProgramId;
        const orderbooks = new Map();
        for (let [productName, { index: index , product: product  }] of $149c1bd638913645$var$Manifest.GetProductsOfMPG(mpg)){
            const meta = $149c1bd638913645$var$productToMeta(product);
            if (aaob_id === null) {
    ...
    const manifest = new $149c1bd638913645$var$Manifest({
        rpc: rpc,
        wallet: wallet,
        ...
        aaob_id: aaob_id,
        dex_id: $149c1bd638913645$var$DEX_ID,
        fees_id: fees_id,
        risk_id: risk_id,
        mpgs: mpgs,
        creationTime: Date.now()
    });
    $149c1bd638913645$var$rpc2manifest.set(key, manifest);
    console.debug("cached manifest", rpc);
    console.debug("got manifest", manifest);
    return manifest;
}
async function $149c1bd638913645$var$getAccountAtDate(publicKey, date) {
    let paramsStr = `?timestamps=${date.getTime()}`;
    const result = await $149c1bd638913645$var$_getAccountAt(publicKey, paramsStr);
    return {
        date: new Date(result[0]),
        data: result[1]
    };
}
async function $149c1bd638913645$var$getAccountAtSlot(publicKey, slot) {
...
async function $149c1bd638913645$var$_getAccountAt(publicKey, paramsStr) {
    const SOLRAY_SECRET = $hgUW1$process?.env?.API_SECRET ?? "";
    ...
    return [
        result[0],
        $149c1bd638913645$require$Buffer.from(bytes)
    ];
}
function $149c1bd638913645$var$bytesToString(bytes) {
    return bytes.map((c)=>String.fromCharCode(c)).join("");
}
function $149c1bd638913645$var$ten() {
    return new (0, $hgUW1$bnjs)(10);
}
function $149c1bd638913645$var$zero() {
    return new (0, $hgUW1$bnjs)(0);
}
function $149c1bd638913645$var$one() {
    return new (0, $hgUW1$bnjs)(1);
}
function $149c1bd638913645$var$negativeOne() {
    return new (0, $hgUW1$bnjs)(-1);
}
class $149c1bd638913645$var$Fractional {
    constructor(m, exp){
        this.m = m;
        this.exp = exp;
        this._isNan = false;
    ...
    static New(m, exp) {
        return new $149c1bd638913645$var$Fractional(new (0, $hgUW1$bnjs)(m), new (0, $hgUW1$bnjs)(exp));
    }
    static From(simple) {
        return $149c1bd638913645$var$Fractional.New(simple.m, simple.exp);
    }
    static FromString(s) {
        if (isNaN(parseFloat(s))) return $149c1bd638913645$var$Fractional.Nan();
        const i = s.indexOf(".");
        if (i < 0) return new $149c1bd638913645$var$Fractional(new (0, $hgUW1$bnjs)(s.replace(/\./g, "")), new (0, $hgUW1$bnjs)(0));
        return new $149c1bd638913645$var$Fractional(new (0, $hgUW1$bnjs)(s.replace(/\./g, "")), new (0, $hgUW1$bnjs)(s.length - i - 1));
    }
    static Zero() {
        return $149c1bd638913645$var$Fractional.New(0, 0);
    }
    static One() {
        return $149c1bd638913645$var$Fractional.New(1, 0);
    }
    static NegativeOne() {
        return $149c1bd638913645$var$Fractional.New(-1, 0);
    }
    static NoBidPrice() {
        return $149c1bd638913645$var$Fractional.New(-9007199254740991, 0); // 64 bits cannot fit in BN :(
    }
    static NoAskPrice() {
        return $149c1bd638913645$var$Fractional.New(9007199254740991, 0); // 64 bits cannot fit in BN :(
    }
    static Nan() {
        let f = $149c1bd638913645$var$Fractional.Zero();
        f._isNan = true;
        return f;
    }
    isNan() {
    ...
    toDecimal() {
        ...
        if (r.m.bitLength() > 53) {
            ...
            r = r.round_down(r.exp.sub(new (0, $hgUW1$bnjs)(Math.ceil(bitDiff / 10 * 3))));
        }
        return r.m.toNumber() / Math.pow(10, r.exp.toNumber());
    }
    // scale multiplies returns a new Fractional with this.m multiplied by c
    scale(c) {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        return new $149c1bd638913645$var$Fractional(this.m.mul(new (0, $hgUW1$bnjs)(c)), this.exp);
    }
    // scale multiplies returns a new Fractional with this.m divided by c
    scaledown(c) {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        return new $149c1bd638913645$var$Fractional(this.m.div(new (0, $hgUW1$bnjs)(c)), this.exp);
    }
    add(other) {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        const cmp = this.exp.cmp(other.exp);
        if (cmp == 0) return new $149c1bd638913645$var$Fractional(this.m.add(other.m), this.exp);
        else if (cmp < 0) return new $149c1bd638913645$var$Fractional(this.round_up(other.exp).m.add(other.m), other.exp);
        else return new $149c1bd638913645$var$Fractional(other.round_up(this.exp).m.add(this.m), this.exp);
    }
    round_up(newExp) {
        if (newExp.lt(this.exp)) throw new Error("cannot use Fractional.round_up to round down");
        return new $149c1bd638913645$var$Fractional(this.m.mul($149c1bd638913645$var$ten().pow(newExp.sub(this.exp))), newExp);
    }
    round_down(newExp) {
        if (newExp.gt(this.exp)) throw new Error("cannot use Fractional.round_down to round up");
        return new $149c1bd638913645$var$Fractional(this.m.div($149c1bd638913645$var$ten().pow(this.exp.sub(newExp))), newExp);
    }
    sub(other) {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        return this.add(new $149c1bd638913645$var$Fractional(other.m.mul($149c1bd638913645$var$negativeOne()), other.exp));
    }
    sign() {
        if (this._isNan) return new (0, $hgUW1$bnjs)(1);
        return this.m.isNeg() ? new (0, $hgUW1$bnjs)(-1) : new (0, $hgUW1$bnjs)(1);
    }
    sqrt() {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        let m = this.m;
        let exp = this.exp;
        if (!exp.umod(new (0, $hgUW1$bnjs)(2)).isZero()) {
            exp = exp.add(new (0, $hgUW1$bnjs)(1));
            m = m.mul(new (0, $hgUW1$bnjs)(10));
        }
        m = m.mul(new (0, $hgUW1$bnjs)(1000000));
        exp = exp.add(new (0, $hgUW1$bnjs)(6));
        return new $149c1bd638913645$var$Fractional($149c1bd638913645$var$bnSqrt(this.m), exp.sub(new (0, $hgUW1$bnjs)(9))).reduced();
    }
    abs() {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        return new $149c1bd638913645$var$Fractional(this.m.abs(), this.exp);
    }
    div(other) {
        if (this._isNan || other._isNan || other.isZero()) return $149c1bd638913645$var$Fractional.Nan();
        const sign = this.sign();
        const otherSign = other.sign();
        const exp = this.exp.sub(other.exp);
        const shift = !this.exp.isNeg() ? $149c1bd638913645$var$ten() : $149c1bd638913645$var$ten().sub(this.exp);
        const dividend = this.m.abs().mul($149c1bd638913645$var$ten().pow(shift));
        const divisor = other.m.abs();
        const quotient = dividend.div(divisor);
        const newExp = exp.add(shift);
        const newSign = sign.mul(otherSign);
        return new $149c1bd638913645$var$Fractional(newSign.mul(quotient), newExp).reduced();
    }
    mul(other) {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        const r1 = this.reduced();
        const r2 = other.reduced();
        return new $149c1bd638913645$var$Fractional(r1.m.mul(r2.m), r1.exp.add(r2.exp));
    }
    reduced() {
        if (this._isNan) return $149c1bd638913645$var$Fractional.Nan();
        if (this.exp.isZero()) return new $149c1bd638913645$var$Fractional(this.m, this.exp);
        if (this.m.isZero()) return $149c1bd638913645$var$Fractional.New(0, 0);
        let m = this.m;
        let exp = this.exp;
        while(m.umod($149c1bd638913645$var$ten()).isZero() && exp.gt($149c1bd638913645$var$zero())){
            m = m.div($149c1bd638913645$var$ten());
            exp = exp.sub($149c1bd638913645$var$one());
        }
        return new $149c1bd638913645$var$Fractional(m, exp);
    }
    ...
    cmp(other) {
        ...
        return r1.m.mul(new (0, $hgUW1$bnjs)(10).pow(r2.exp)).cmp(r2.m.mul(new (0, $hgUW1$bnjs)(10).pow(r1.exp)));
    }
    lt(other) {
...
const $149c1bd638913645$var$NUM_LIQUIDATION_STDS = $149c1bd638913645$var$Fractional.New(15, 1);
const $149c1bd638913645$var$NUM_UNHEALTHY_STDS = $149c1bd638913645$var$Fractional.New(3, 0);
function $149c1bd638913645$var$getEnumVariantAsString(someEnum) {
    const props = Object.getOwnPropertyNames(someEnum);
    if (props.length !== 1) return "Invalid Enum";
    return props[0];
}
function $149c1bd638913645$var$getPriceDecimals(meta) {
...
function $149c1bd638913645$var$productStatus(p, productArray) {
    if (p.hasOwnProperty("outright")) return $149c1bd638913645$var$getEnumVariantAsString(p.outright.outright.productStatus);
    for (const [i, leg] of p.combo.combo.legs.slice(0, p.combo.combo.numLegs.toNumber()).entries()){
        const status = $149c1bd638913645$var$productStatus(productArray[leg.productIndex.toNumber()], productArray);
        if (status !== "initialized") return status;
    }
    return "initialized";
}
function $149c1bd638913645$var$productToMeta(p) {
    if (p.hasOwnProperty("outright")) return p.outright.outright.metadata;
    else return p.combo.combo.metadata;
}
let $149c1bd638913645$var$TraderUpdateType;
...
})($149c1bd638913645$var$TraderUpdateType || ($149c1bd638913645$var$TraderUpdateType = {}));
// one Trader per trader risk group
// one or more trader risk groups per market product group
class $149c1bd638913645$var$Trader {
    ...
    async timeTravelToDate(toDate) {
        ...
        let result = await $149c1bd638913645$var$getAccountAtDate(this.marketProductGroup, toDate);
        this.mpg = await this.manifest.getMPGFromData(result.data);
        this.mpgDate = result.date;
        result = await $149c1bd638913645$var$getAccountAtDate(this.traderRiskGroup, toDate);
        this.trg = await this.manifest.getTRGFromData(result.data);
        this.trgDate = result.date;
        result = await $149c1bd638913645$var$getAccountAtDate(this.trg.riskStateAccount, toDate);
        this.varianceCache = result.data;
        this.riskDate = result.date;
        result = await $149c1bd638913645$var$getAccountAtDate(this.markPricesAccount, toDate);
    ...
    getProducts() {
        return $149c1bd638913645$var$Manifest.GetProductsOfMPG(this.mpg);
    }
    getPositions() {
        let m = new Map();
        for (let p of this.trg.traderPositions){
            if (p.productKey.toBase58() === $149c1bd638913645$var$UNINITIALIZED || "uninitialized" in p.tag) continue;
            m.set($149c1bd638913645$var$productToMeta(this.mpg.marketProducts.array[p.productIndex]).name.map((c)=>String.fromCharCode(c)).join(""), $149c1bd638913645$var$Fractional.From(p.position));
        }
        return m;
    }
    async newOrder(productIndex, isBid, limitPrice, maxBaseQty, { onGettingBlockHashFn: onGettingBlockHashFn , onGotBlockHashFn: onGotBlockHashFn , onTxSentFn: onTxSentFn , onTxSuccessFn: onTxSuccessFn , onFailFn: onFailFn  } = {
        ...
        const params = {
            ...
            matchLimit: matchLimit ?? new (0, $hgUW1$bnjs)(16),
            limitPrice: {
                m: limitPrice.m,
                exp: limitPrice.exp
            },
            referrerFeeBps: {
                m: referrerFeeBps ? referrerFeeBps.m : new (0, $hgUW1$bnjs)(0),
                exp: referrerFeeBps ? referrerFeeBps.exp : new (0, $hgUW1$bnjs)(0)
            },
            clientOrderId: clientOrderId ?? new (0, $hgUW1$bnjs)(0)
        };
        try {
            ...
            //     }
            // );
            const tx = new (0, $hgUW1$Transaction)().add((0, $hgUW1$ComputeBudgetProgram).setComputeUnitLimit({
                units: $149c1bd638913645$var$MAX_COMPUTE_UNITS
            ...
            .add(this.manifest.fields.dexProgram.instruction.newOrder(params, {
                accounts: {
                    ...
                    riskAndFeeSigner: $149c1bd638913645$var$Manifest.GetRiskAndFeeSigner(this.marketProductGroup),
    ...
    getUpdateMarkPricesInstruction(products) {
        ...
        for (let p of products){
            ...
            const meta = $149c1bd638913645$var$productToMeta(p);
    ...
    async initializePrintTrade(isBid, size, price, counterparty) {
        ...
        const params = {
            productIndex: new (0, $hgUW1$bnjs)(productAndIndex.index),
            side: side,
            size: {
                m: size.m,
                exp: size.exp
        ...
        const printTrade = (0, $hgUW1$PublicKey).findProgramAddressSync([
            $149c1bd638913645$require$Buffer.from("print_trade", "utf-8"),
            productPk.toBuffer(),
    ...
    getOpenOrders(productNames) {
        ...
        for (const [name, { index: index , product: product  }] of this.getProducts()){
            ...
            const metadata = $149c1bd638913645$var$productToMeta(product);
            const tickSize = $149c1bd638913645$var$Fractional.From(metadata.tickSize);
            const priceOffset = $149c1bd638913645$var$Fractional.From(metadata.priceOffset);
            const baseDecimals = new (0, $hgUW1$bnjs)(metadata.baseDecimals);
            ...
            if (order.prev.toNumber() !== $149c1bd638913645$var$SENTINEL) throw new Error("openOrders state is invalid. expected first order.prev === SENTINEL\norder: " + JSON.stringify(order));
            while(ptr !== $149c1bd638913645$var$SENTINEL){
                order = this.trg.openOrders.orders[ptr];
                if (order.id.isZero()) throw new Error("expected order id !== 0. order: " + JSON.stringify(order));
                orders.add(new $149c1bd638913645$var$Order(order.id, trimmedName, index, $149c1bd638913645$var$Manifest.orderIdToDexPrice(order.id, tickSize, priceOffset), new $149c1bd638913645$var$Fractional(new (0, $hgUW1$bnjs)(order.qty), baseDecimals), $149c1bd638913645$var$Manifest.orderIdIsBid(order.id)));
    ...
    async cancelAllOrders(productNames, isUseCache = false) {
        ...
        for (const [name, { index: index , product: product  }] of this.getProducts()){
            ...
            if (order.prev.toNumber() !== $149c1bd638913645$var$SENTINEL) throw new Error("openOrders state is invalid. expected first order.prev === SENTINEL. order: " + JSON.stringify(order));
            while(ptr !== $149c1bd638913645$var$SENTINEL){
                order = this.trg.openOrders.orders[ptr];
                if (order.id.isZero()) throw new Error("expected order id !== 0. order: " + JSON.stringify(order));
    ...
    async batchCancelOrders(product, orderIds, { onGettingBlockHashFn: onGettingBlockHashFn , onGotBlockHashFn: onGotBlockHashFn , onTxSentFn: onTxSentFn , onTxSuccessFn: onTxSuccessFn , onFailFn: onFailFn  } = {
        ...
        const accounts = {
            ...
            riskOutputRegister: this.mpg.riskOutputRegister,
            traderRiskStateAcct: this.trg.riskStateAccount,
            riskSigner: $149c1bd638913645$var$Manifest.GetRiskAndFeeSigner(this.marketProductGroup),
            covarianceMetadata: this.manifest.getRiskS(this.marketProductGroup),
            correlationMatrix: this.manifest.getRiskR(this.marketProductGroup),
            markPrices: this.markPricesAccount
        };
        let varianceCacheUpdated = false;
        for(let i = 0; i < orderIds.length; i += $149c1bd638913645$var$MAX_CANCELS_PER_TX){
            const lastChunk = i + $149c1bd638913645$var$MAX_CANCELS_PER_TX >= orderIds.length;
            const orderIdsChunk = orderIds.slice(i, i + $149c1bd638913645$var$MAX_CANCELS_PER_TX);
            try {
                const tx = new (0, $hgUW1$Transaction)().add((0, $hgUW1$ComputeBudgetProgram).setComputeUnitLimit({
                    units: $149c1bd638913645$var$MAX_COMPUTE_UNITS
                }));
    ...
    async deposit(usdcAmount) {
        try {
            const tradersVaultATA = await $149c1bd638913645$var$Manifest.GetATAFromMPGObject(this.mpg, this.manifest.fields.wallet.publicKey);
            const vaultNotMint = (0, $hgUW1$PublicKey).findProgramAddressSync([
                $149c1bd638913645$require$Buffer.from("market_vault", "utf-8"),
                this.marketProductGroup.toBuffer()
            ], new (0, $hgUW1$PublicKey)($149c1bd638913645$var$DEX_ID))[0];
            const capitalLimits = (0, $hgUW1$PublicKey).findProgramAddressSync([
                $149c1bd638913645$require$Buffer.from("capital_limits_state", "utf-8"),
                this.marketProductGroup.toBuffer()
            ], new (0, $hgUW1$PublicKey)($149c1bd638913645$var$DEX_ID))[0];
            const accounts = {
                tokenProgram: (0, $hgUW1$TOKEN_PROGRAM_ID),
                user: this.manifest.fields.wallet.publicKey,
    ...
    async withdraw(usdcAmount) {
        try {
            ...
            if (!tradersVaultATAInfo) {
                createAtaIx = (0, $hgUW1$createAssociatedTokenAccountInstruction)(this.manifest.fields.wallet.publicKey, tradersVaultATA, this.manifest.fields.wallet.publicKey, this.mpg.vaultMint);
            ...
            const tx = new (0, $hgUW1$Transaction)();
    ...
    streamUpdates(onUpdateFn) {
        ...
        this.trgSocket = this.manifest.accountSubscribe(this.traderRiskGroup, async (data, manifest)=>await this.manifest.getTRGFromData(data), ((trg, slot)=>{
            ...
            this.trgSlot = slot;
            onUpdateFn($149c1bd638913645$var$TraderUpdateType.TRG);
        }).bind(this));
        this.mpgSocket = this.manifest.accountSubscribe(this.marketProductGroup, async (data, manifest)=>await this.manifest.getMPGFromData(data), ((mpg)=>{
            this.mpg = mpg;
            onUpdateFn($149c1bd638913645$var$TraderUpdateType.MPG);
        }).bind(this));
        this.riskSocket = this.manifest.accountSubscribe(this.trg.riskStateAccount, async (data, manifest)=>data, ((varianceCache)=>{
            ...
            onUpdateFn($149c1bd638913645$var$TraderUpdateType.Risk);
        }).bind(this));
        this.markPricesSocket = this.manifest.accountSubscribe(this.markPricesAccount, async (data, manifest)=>await this.manifest.getMarkPricesFromData(data), ((markPrices)=>{
            ...
            onUpdateFn($149c1bd638913645$var$TraderUpdateType.MarkPrices);
        }).bind(this));
    }
    async updateRisk() {
    ...
    getRiskNumber(offset, size, isSigned = true) {
        return $149c1bd638913645$var$Manifest.GetRiskNumber(this.varianceCache, offset, size, isSigned);
    // if (isSigned) {
    ...
    getPositionValue() {
        if (this.varianceCache) return $149c1bd638913645$var$Manifest.FromFastInt(this.getRiskNumber(24, 16));
    }
    getTradedVariance() {
        if (this.varianceCache) return $149c1bd638913645$var$Manifest.FromFastInt(this.getRiskNumber(40, 16));
    }
    getOpenOrderVariance() {
        if (this.varianceCache) return $149c1bd638913645$var$Manifest.FromFastInt(this.getRiskNumber(56, 16));
    }
    getCashBalance() {
        return $149c1bd638913645$var$Fractional.From(this.trg.cashBalance);
    }
    getPendingCashBalance() {
        return $149c1bd638913645$var$Fractional.From(this.trg.pendingCashBalance);
    }
    getNetCash() {
    ...
    getTotalDeposited() {
        return $149c1bd638913645$var$Fractional.From(this.trg.totalDeposited);
    }
    getTotalWithdrawn() {
        return $149c1bd638913645$var$Fractional.From(this.trg.totalWithdrawn);
    }
    ...
    getRequiredMaintenanceMargin() {
        if (this.varianceCache) {
            const portfolioStd = (0, $hgUW1$bnjs).max(this.getTradedVariance(), this.getOpenOrderVariance()).sqrt();
            return portfolioStd.mul($149c1bd638913645$var$NUM_LIQUIDATION_STDS);
        }
    }
    ...
    getRequiredInitialMargin() {
        if (this.varianceCache) {
            const portfolioStd = (0, $hgUW1$bnjs).max(this.getTradedVariance(), this.getOpenOrderVariance()).sqrt();
            return portfolioStd.mul($149c1bd638913645$var$NUM_UNHEALTHY_STDS);
        }
    }
    ...
    getExcessMaintenanceMargin() {
        if (this.varianceCache) {
            const portfolioStd = (0, $hgUW1$bnjs).max(this.getTradedVariance(), this.getOpenOrderVariance()).sqrt();
            return this.getPortfolioValue().sub(portfolioStd.mul($149c1bd638913645$var$NUM_LIQUIDATION_STDS));
        }
    }
    ...
    getExcessInitialMargin() {
        if (this.varianceCache) {
            const portfolioStd = (0, $hgUW1$bnjs).max(this.getTradedVariance(), this.getOpenOrderVariance()).sqrt();
            return this.getPortfolioValue().sub(portfolioStd.mul($149c1bd638913645$var$NUM_UNHEALTHY_STDS));
        }
    }
    ...
    async update(isUpdateMPG = true) {
        ...
        if (isUpdateMPG) {
            this.marketProductGroup = new (0, $hgUW1$PublicKey)(this.trg.marketProductGroup.toBase58());
...
function $149c1bd638913645$var$createDexProgram(provider) {
    return new (0, $hgUW1$Program)($149c1bd638913645$var$DEX_IDL, new (0, $hgUW1$PublicKey)($149c1bd638913645$var$DEX_ID), provider);
}
function $149c1bd638913645$var$createInstrumentsProgram(provider) {
    return new (0, $hgUW1$Program)($149c1bd638913645$var$INSTRUMENTS_IDL, new (0, $hgUW1$PublicKey)($149c1bd638913645$var$INSTRUMENTS_ID), provider);
}
function $149c1bd638913645$var$createRiskProgram(provider) {
    return new (0, $hgUW1$Program)($149c1bd638913645$var$RISK_IDL, new (0, $hgUW1$PublicKey)($149c1bd638913645$var$RISK_ID), provider);
}
var $149c1bd638913645$export$2e2bcd8739ae039 = {
    bytesToString: $149c1bd638913645$var$bytesToString,
    Fractional: $149c1bd638913645$var$Fractional,
    getManifest: $149c1bd638913645$var$getManifest,
    getPriceDecimals: $149c1bd638913645$var$getPriceDecimals,
    Manifest: $149c1bd638913645$var$Manifest,
    productStatus: $149c1bd638913645$var$productStatus,
    productToMeta: $149c1bd638913645$var$productToMeta,
    rpc2manifest: $149c1bd638913645$var$rpc2manifest,
    Trader: $149c1bd638913645$var$Trader,
    TraderUpdateType: $149c1bd638913645$var$TraderUpdateType
};

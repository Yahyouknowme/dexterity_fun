...
import { Idl, Program, AnchorProvider, web3, Wallet } from '@project-serum/anchor';
import {
...
import { Commitment, ComputeBudgetProgram, ConfirmOptions, Connection, PublicKey, Keypair, SystemProgram, Transaction, TransactionInstruction, VersionedTransaction } from '@solana/web3.js';
...
class Manifest {
    ...
    getRiskS(mpg: PublicKey): PublicKey {
        return PublicKey.findProgramAddressSync([Buffer.from("s", "utf-8"), mpg.toBuffer()], new PublicKey(this.fields.risk_id))[0];
    }

    getRiskR(mpg: PublicKey): PublicKey {
        return PublicKey.findProgramAddressSync([Buffer.from("r", "utf-8"), mpg.toBuffer()], new PublicKey(this.fields.risk_id))[0];
    }
    ...
    getMarkPricesAccount(mpg: PublicKey): PublicKey {
        return PublicKey.findProgramAddressSync([Buffer.from("mark_prices", "utf-8"), mpg.toBuffer()], new PublicKey(this.fields.risk_id))[0];
    }
    ...
    async getCovarianceMetadata(marketProductGroup: PublicKey): Promise<CovarianceMetadata> {
    ...
    static orderIdToDexPrice(id: BN, tickSize: Fractional, offset: Fractional): Fractional {
        return new Fractional(id.shrn(64 + 32), new BN(0)).div(tickSize).sub(offset);
    }
    ...
    streamBooks(product, marketState, onBookFn, onMarkPricesFn = null): { asksSocket: ReliableWebSocket, bidsSocket: ReliableWebSocket, markPricesSocket: ReliableWebSocket } {
        ...
        if (onMarkPricesFn !== null) {
            ...
            markPricesSocket = this.accountSubscribe(
                this.getMarkPricesAccount(mpgPk),
...
async function getManifest(rpc: string, useCache = false, wallet: DexterityWallet | Wallet): Promise<Manifest> {
    ...
    let fees_id = null;
    let risk_id = null;
    const accounts = await connection.getParsedProgramAccounts(DEX_ID, { filters: [ { dataSize: MPG_SIZE } ] });
    const mpgs = new Map();
    ...
    if (risk_id == null || fees_id == null || aaob_id == null) {
...
class Trader {
    ...
    async newOrder(productIndex, isBid, limitPrice: Fractional, maxBaseQty: Fractional,
                   { onGettingBlockHashFn, onGotBlockHashFn, onTxSentFn, onTxSuccessFn, onFailFn }
        = { onGettingBlockHashFn: null, onGotBlockHashFn: null, onTxSentFn: null, onTxSuccessFn: null, onFailFn: null},isIOC=false,
                   referrerTrg=null, referrerFeeBps=null, clientOrderId=null,
                   matchLimit=null
                  ) {
        ...
        if (product === null) {
            throw new Error('could not place new order because no product with that index exists');
        }
        ...
        try {
            ...
            const tx = new Transaction()
                .add(ComputeBudgetProgram.setComputeUnitLimit({ units: MAX_COMPUTE_UNITS }))
                // .add(this.getUpdateMarkPricesInstruction())
                .add(
                this.manifest.fields.dexProgram.instruction.newOrder(params, { accounts: {
                    // @ts-ignore
    ...
    async getUpdateVarianceCacheInstruction() {
        ...
        try {
            return await this.manifest.fields.dexProgram.methods.updateVarianceCache()
            // @ts-ignore
                .accounts(accounts)
                .instruction();
        } catch (e) {
    ...
    getUpdateMarkPricesInstruction(products: Array<Product>) {
    ...
    getOpenOrders(productNames): Set<Order> {
        ...
        for (const [name, { index, product }] of this.getProducts()) {
            ...
            if (order.prev.toNumber() !== SENTINEL) { // sunchokes
    ...
    async batchCancelOrders(product, orderIds,
                            { onGettingBlockHashFn, onGotBlockHashFn, onTxSentFn, onTxSuccessFn, onFailFn }
        = { onGettingBlockHashFn: null, onGotBlockHashFn: null, onTxSentFn: null, onTxSuccessFn: null, onFailFn: null },
                            clientOrderId=null
                           ) {
        ...
        for (let i = 0; i < orderIds.length; i += MAX_CANCELS_PER_TX) {
            const lastChunk = i + MAX_CANCELS_PER_TX >= orderIds.length;
            const orderIdsChunk = orderIds.slice(i, i + MAX_CANCELS_PER_TX);
            try {
                const tx = new Transaction()
                    .add(ComputeBudgetProgram.setComputeUnitLimit({ units: MAX_COMPUTE_UNITS }));
                for (let y = 0; y < orderIdsChunk.length; y++) {
                    tx.add(
                        await this.manifest.fields.dexProgram.methods
                            .cancelOrder({ orderId: orderIdsChunk[y], noErr: true, clientOrderId: clientOrderId ?? new BN(0) })
                        // @ts-ignore
                            .accounts(accounts)
                            .signers([wallet])
                            .instruction()
                ...
                if (onTxSentFn) {
                    onTxSentFn(cancelOrdersSig);
                }
                await connection.confirmTransaction(cancelOrdersSig);
                if (onTxSuccessFn) {
                    onTxSuccessFn(cancelOrdersSig);
                }
            } catch (e) {
                if (onFailFn) {
                    onFailFn(e);
        ...
        if (!varianceCacheUpdated) {
            await this.updateVarianceCache();
        }
    }

    async cancelOrders(productIndex, orderIds, isUseCache = true,
                       { onGettingBlockHashFn, onGotBlockHashFn, onTxSentFn, onTxSuccessFn, onFailFn }
        = { onGettingBlockHashFn: null, onGotBlockHashFn: null, onTxSentFn: null, onTxSuccessFn: null, onFailFn: null },
                       clientOrderId=null
                      ) {
        ...
        for (const [name, { index, product }] of this.getProducts()) {
            ...
            await this.batchCancelOrders(product, orderIds, { onGettingBlockHashFn, onGotBlockHashFn, onTxSentFn, onTxSuccessFn, onFailFn }, clientOrderId);
            return;
        }
    }

    async deposit(usdcAmount: Fractional) {
        try {
            ...
            await this.manifest.fields.dexProgram.methods
                .depositFunds({ quantity: { m: usdcAmount.m,  exp: usdcAmount.exp } })
                .accounts(accounts)
                .rpc();
        } catch (e) {
            console.error(e);
    ...
    streamUpdates(onUpdateFn) {
        ...
        this.trgSocket = this.manifest.accountSubscribe(
            ...
            ((trg, slot) => { this.trg = trg; console.log('slot', slot); this.trgSlot = slot; onUpdateFn(TraderUpdateType.TRG); }).bind(this),
        );
        ...
        this.mpgSocket = this.manifest.accountSubscribe(
            ...
            (mpg => { this.mpg = mpg; onUpdateFn(TraderUpdateType.MPG); }).bind(this),
        );
